...
13. 通常用户上传时from表单若包含附件或者含有用户隐私信息（如密码）时，提交方式不得使用GET形式，而应当用POST形式
    POST请求会将用户上传的信息会包含在请求的消息正文中，对此我们在HttpRequest中要解析消息正文
    
    这里我们先解决form表单上传注册信息的post解析，实际上POST在上传用户注册信息时，
    只将原URL中？右侧的内容挪到了请求的消息正文中而已
    
    我们可以通过分析请求的消息头是否含有Content-Type与
    Content-Length就可以得知该请求是否含有消息正文了
    
    1:完成HttpRequest的parseContent方法，解析消息正文
    	首先根据消息头判断是否含有Content-Length，若寒有则可以判断出当前请求时含有消息正文的
    	按照对应长度通过输出流读取出这些字节
    	
    	再来通过消息头Content-Type判断这个消息正文内容是什么类型数据，这里只判断是否为form表单提交的数据
    	而对应的Content-Type的值是固定的：
    	application/x-www-form-urlencoded
    	如果是form表单数据，就将读取到的字节转换为字符串，而他这个字符串的内容与原来使用GET形式提交时地址栏中
    	“？”右侧内容是一致的
